...
class OperatorReportJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $totals = DB::table('wallet_transactions as wt')
            ...
            ->groupBy('wt.user_id', 'u.mobile', 'u.name', 'p.national_code', 'p.address', 'p.postal_code')
            ->get()
            ->keyBy('user_id');
        ...
        $results = $totals->map(function ($item, $userId) use ($operatorData) {
            ...
            return [
                ...
                'date' => new UTCDateTime(Carbon::parse($this->date)),
                'created_at' => now(),
                'updated_at' => now(),
            ];
        })->values()->toArray();
